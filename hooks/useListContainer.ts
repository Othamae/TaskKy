...
export const UseListContainer = ({ boardId, list }: { boardId: string; list: ListWithCards[] }) => {
	...
	const onDragEnd = (result: any) => {
		...
		if (type === 'list') {
			const newOrderedList = reorder(orderedList, source.index, destination.index).map((item, index) => ({
				...item,
				order: index,
			}))
